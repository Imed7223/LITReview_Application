...
def dashboard(request):
    ...
    # Utilisez Q directement (sans models.)
    followed_users = request.user.following.values_list('followed_user', flat=True)
    feed_tickets = Ticket.objects.filter(Q(user__in=followed_users) | Q(user=request.user))
    feed_reviews = Review.objects.filter(Q(user__in=followed_users) |
                                         Q(user=request.user) |
                                         Q(ticket__user=request.user))
    feed_tickets = feed_tickets.annotate(content_type=Value('TICKET', CharField()))
    feed_reviews = feed_reviews.annotate(content_type=Value('REVIEW', CharField()))
    posts = sorted(chain(feed_tickets, feed_reviews), key=lambda post: post.time_created, reverse=True)
...
def create_review(request, ticket_id=None):
    ...
    if request.method == 'POST':
        ...
        if review_form.is_valid() and (ticket or ticket_form and ticket_form.is_valid()):
            if not ticket:
    ...
    return render(request, 'reviews/review_form.html', {'review_form': review_form, 'ticket_form': ticket_form, 'ticket': ticket})
...
def follow_user(request):
    ...
    return render(request, 'reviews/follow_user.html', {'error_message': error_message})
...
def edit_review(request, review_id):
    ...
    if request.method == 'POST' and form.is_valid():
        form.save()
        messages.success(request, "Votre critique a été modifié !")
        return redirect('dashboard')
    return render(request, 'reviews/review_form.html', {'review_form': form, 'ticket': review.ticket})
